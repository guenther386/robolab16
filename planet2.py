...
gs.mode = 'GYRO-CAL'
gs.mode = 'GYRO-ANG'
client = mqtt.Client(client_id="025", clean_session=False, protocol=mqtt.MQTTv31)
posx = 0
posy = 0
head = 0
last = 0
...
def msg(client, data, message):
    ...
    if string == "":
        ...
        string = '{}'.format(message.payload.decode('utf-8'))
        print(string)
        ev3.Sound.tone([(666, 100, 100), (800, 200)])
        ...
        if got != sent:
            ev3.Sound.tone([(666, 50, 50), (800, 100)])
        strlist = got.split()
        if strlist[0] == "target":
            target = (int(strlist[1])-int(offsetx), int(strlist[2])-int(offsety))
...
def start():
    ...
    client.username_pw_set('025', password='uNq2OTFrLn')
    client.connect('stream-006.inf.tu-dresden.de', port=8883)
    client.subscribe('explorer/025', qos=1)
    client.loop_start()
    client.publish('explorer/025', payload="ready", qos=1)
    time.sleep(2)
    if string == "" or string == "ready":
        ev3.Sound.tone([(400, 100, 100), (333, 200)])
    else:
        client.subscribe(string, qos=1)
    return None
...
def publish(out, in_):
    ...
    global offsety
    global sent
    x, y = nodes[current].pos
    lx, ly = nodes[last].pos
    x += int(offsetx)
    y += int(offsety)
    lx += int(offsetx)
    ly += int(offsety)
    load = "path {} {} {} {} {} {}".format(lx, ly, out, x, y, in_)
    client.publish(string, payload="{}".format(load), qos=1)
    sent = load
    print(sent)
    return None
...
def follow_to(i):
    ...
    l=1
    while k < i:
        if nodes[last].list[k] is not None:
            l += 1
        k += 1
    k=0
    m=1
    while k < (direct-2)%4:
        if nodes[current].list[k] is not None:
            m += 1
        k += 1
    if string != "" and string != "ready":
        publish(l, m)
        time.sleep(2)
    direct = 0
    if 0 <= angle()%360 < 23:
